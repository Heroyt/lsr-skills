...
**Core 0.6 / ORM 0.4** widen their public logger contracts to `Psr\Log\LoggerInterface`; this is not part of the published compatibility patches. Core's explicit service reference accepts a PSR logger, and ORM accepts a custom provider returning one. Default LSR loggers remain unchanged. Audit concrete helper calls and inherited property types before opting an application into those minors, and use the [compatibility set](../../README.md#psr-logger-compatibility-set) for path testing.
