...
- Published 0.3.25 uses `Lsr\Orm\Logging\ModelLoggerProviderInterface::getLogger(string $modelClass): Lsr\Logging\Logger`, with a `class-string<Model>` input. **0.4 (unreleased)** widens that return, the model/repository getters and protected property to `Psr\Log\LoggerInterface`. Existing concrete providers remain covariant; generic PSR providers must not be installed against 0.3.25.
...
- Use `orm.logging.provider: @applicationModelLoggerProvider` for custom routing, including an existing shared logger. Version 0.3.25 requires a concrete LSR result; unreleased 0.4 accepts any PSR logger and returns the exact object without wrapping or injecting model context. Do not combine a custom provider with `storage` or `directory`; those configure the built-in provider.
...
- Before adopting 0.4, remove or widen inherited protected logger redeclarations to exactly `LoggerInterface`, and migrate `exception()` / `logDb()` calls through model getters. Those helpers remain on concrete LSR loggers, not the generic model contract. Preserve the old error/debug records rather than silently dropping helper calls or merging records; the package README contains the migration example.
